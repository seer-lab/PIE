...
class LifecycleParser: 
  ...
  def get_intervals(self, f, interval_name): 
    intervals = []
    start = -1
    ...
    for document, commit_number in zip(self.documents, range(len(self.documents))): 
      if f(document):
        ...
        intervals.append({'instance': interval_name, 'start': start, 'end': commit_number -1})
        start = -1
    return intervals
  ...
  def file_exists(self, file): 
    def f(document):
      if file in analysis_tools.get_file_names(document['file_list']):
        return True 
      return False 
    return f
  ...
  def git_pattern_instance_timeline(self, pattern_instance, pattern):
    pattern_intervals = self.get_intervals(self.document_contains_pattern(pattern_instance, pattern), 'Design Pattern')
    file_names = pattern_instance.split('-')

    file_intervals = [] + pattern_intervals
